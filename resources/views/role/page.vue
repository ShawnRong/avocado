<template>
  <div class="app-container">
    <div class="filter-container">
      <el-form :inline="true" :model="queryParams"  size="mini">
        <el-input :placeholder="$t('name')" v-model="queryParams.name" style="width: 200px;" class="filter-item"></el-input>
        <el-button type="primary" @click="requestData" icon="el-icon-search" class="filter-item">{{ $t('search') }}</el-button>
        <el-button type="primary" @click="handleAdd" icon="el-icon-plus" class="filter-item">{{ $t('add') }}</el-button>
      </el-form>
    </div>

    <el-table
            :data="tableData"
            border
            style="width: 100%">
      <el-table-column :label="$t('id')" prop="id" sortable="custom" align="center" width="65">
        <template slot-scope="scope">
          <span>{{ scope.row.id }}</span>
        </template>
      </el-table-column>
      <el-table-column
              prop="name"
              align="center"
              :label="$t('name')">
      </el-table-column>
      <el-table-column
              prop="guard_name"
              align="center"
              :label="$t('guardName')">
      </el-table-column>
      <el-table-column
              prop="description"
              align="center"
              :label="$t('description')">
      </el-table-column>
      <el-table-column
              prop="created_at"
              align="center"
              :label="$t('createdAt')">
      </el-table-column>
      <el-table-column
              prop="updated_at"
              align="center"
              :label="$t('updatedAt')">
      </el-table-column>
      <el-table-column
              fixed="right"
              align="center"
              width="300px"
              :label="$t('actions')"
              >
        <template slot-scope="scope">
          <el-button
                  size="mini"
                  type="primary"
                  @click="handleEdit(scope.$index, scope.row)">{{ $t('edit') }}</el-button>
          <el-button
                  size="mini"
                  type="danger"
                  @click="handleDelete(scope.$index, scope.row)">{{ $t('delete') }}</el-button>
        </template>
      </el-table-column>
    </el-table>

    <el-pagination class="table-paginator"
                   @current-change="requestData"
                   :current-page.sync="pagination.currentPage"
                   :page-size="pagination.pageSize"
                   layout="total, prev, pager, next, jumper"
                   :total="pagination.total">
    </el-pagination>


    <el-dialog :title="$t('add')" :visible.sync="dialogAddFormVisible" width="50%">
      <el-form :model="addForm" :rules="rules" ref="addForm">
        <el-form-item :label="$t('name')" prop="name" :label-width="formLabelWidth">
          <el-input v-model="addForm.name"></el-input>
        </el-form-item>
        <el-form-item :label="$t('guardName')" prop="guard_name" :label-width="formLabelWidth">
          <guard-select :nowValue.sync="addForm.guard_name"></guard-select>
        </el-form-item>
        <el-form-item :label="$t('description')" prop="description" :label-width="formLabelWidth">
          <el-input v-model="addForm.description"></el-input>
        </el-form-item>
        <el-form-item :label="$t('assignPermission')" prop="assignPermission" :label-width="formLabelWidth">
          <permission-transfer v-if="dialogAddFormVisible" :assignList="permissionList" :submitData.sync="addForm.permissions"></permission-transfer>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogAddFormVisible = false">{{ $t('cancel') }}</el-button>
        <el-button type="primary" @click="handleAddRole">{{ $t('confirm') }}</el-button>
      </div>
    </el-dialog>


    <el-dialog :title="$t('edit')" :visible.sync="dialogEditFormVisible" width="50%">
      <el-form :model="editForm" :rules="rules" ref="editForm">
        <el-form-item :label="$t('name')" prop="name" :label-width="formLabelWidth">
          <el-input v-model="editForm.name"></el-input>
        </el-form-item>
        <el-form-item :label="$t('guardName')" prop="guard_name" :label-width="formLabelWidth">
          <guard-select :nowValue.sync="editForm.guard_name"></guard-select>
        </el-form-item>
        <el-form-item :label="$t('description')" prop="description" :label-width="formLabelWidth">
          <el-input v-model="editForm.description"></el-input>
        </el-form-item>
        <el-form-item :label="$t('assignPermission')" prop="assignPermission" :label-width="formLabelWidth">
          <permission-transfer v-if="dialogEditFormVisible" :assignList="permissionList" :submitData.sync="editForm.permissions"></permission-transfer>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogEditFormVisible = false">{{ $t('cancel') }}</el-button>
        <el-button type="primary" @click="handleEditRole">{{ $t('confirm') }}</el-button>
      </div>
    </el-dialog>

  </div>
</template>
<script>
import { getRoleList, addRole, editRole, deleteRole } from "../../api/role.js";
import { getPermissionList, loadPermissions } from "../../api/permission.js";
import {
  responseDataFormat,
  tableDefaultData,
  editSuccess,
  addSuccess,
  deleteSuccess
} from "../../utils/tableDataHandle.js";
import GuardSelect from "../../components/Select/Guard";
import PermissionTransfer from "../../components/Transfer/PermissionTransfer";

export default {
  name: "roleIndex",
  components: {
    GuardSelect,
    PermissionTransfer
  },
  data() {
    return {
      ...tableDefaultData(),
      permissionList: [],
      addForm: {
        name: "",
        guard_name: "",
        description: "",
        permissions: []
      },
      editForm: {
        name: "",
        guard_name: "",
        description: "",
        permissions: []
      },
      rules: {
        name: [{ required: true }, { min: 1, max: 255 }],
        guard_name: [{ required: true }, { min: 1, max: 255 }]
      }
    };
  },
  methods: {
    pushList(data) {
      this.editForm.submitPermissions = data;
    },
    requestData() {
      getRoleList({
        ...this.queryParams,
        page: this.pagination.currentPage
      }).then(response => {
        responseDataFormat(response, this);
      });
      loadPermissions().then(response => {
        this.permissionList = response.data.data.map((item, index) => {
          return {
            key: index + 1,
            label: item.name
          };
        });
      });
    },
    handleAddRole() {
      this.$refs["addForm"].validate(valid => {
        if (valid) {
          addRole(this.addForm).then(response => {
            addSuccess(this);
          });
        } else {
          return false;
        }
      });
    },
    handleAdd() {
      this.dialogAddFormVisible = true;
      this.addForm.permissions = [];
    },
    handleEdit(index, row) {
      let tempPermissions = [];
      if (
        typeof row.permissions.data !== "undefined" &&
        row.permissions.data.length > 0
      ) {
        tempPermissions = row.permissions.data.map(permission => permission.id);
      }
      this.editForm = {
        name: row.name,
        guard_name: row.guard_name,
        description: row.description,
        permissions: tempPermissions
      };
      this.nowRowData = { index, row };
      this.dialogEditFormVisible = true;
    },
    handleEditRole() {
      this.$refs["editForm"].validate(valid => {
        if (valid) {
          editRole(this.nowRowData.row.id, this.editForm).then(response => {
            editSuccess(this);
            this.requestData();
          });
        } else {
          return false;
        }
      });
    },
    handleDelete(index, row) {
      deleteRole(row.id).then(response => {
        deleteSuccess(index, this);
      });
    }
  },
  created() {
    this.requestData();
  }
};
</script>

